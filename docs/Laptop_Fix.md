# SimLab X-Ray Client Side - Fix

Files to copy (sans PuTTY install), can be found in S:\Delphi

#### 1. Software Dependencies
- **Windows OS**: Windows 10/11 or Windows Server
- **PuTTY Suite**: Required for SFTP client (PSFTP)
  - Install from: https://www.chiark.greenend.org.uk/~sgtatham/putty/latest.html
  - Typical install path: `C:\Program Files\PuTTY\psftp.exe`
  - Alternative: `C:\Program Files (x86)\PuTTY\psftp.exe` (32-bit)
  - Verify inclusion in system path.

#### 2. SSH Keys (Client)
**Source Location**: `S:\Delph\Client_Fix_Files\`
**Destination Location**: `C:\Users\svc-dentappsnd-sftp\.ssh\`
- (folder will need to be created)

**Required Files**:
- `id_rsa.ppk` - PuTTY private key (PuTTY format, **required**)
- `.ssh.pub` - Public key (for reference/backup)
- `.ssh` - OpenSSH private key (optional, for conversion)

#### 3. Script Files
**Source Location**: `S:\Delph\Client_Fix_Files\`
**Destination Location**: `C:\Simlab\`

**Files**:
- `upload_image.bat` - Main upload script
- `xray.jpg` - Source image file (generated by imaging application)

**Optional CLI Command for manual testing of upload-image**:
- `./upload_image.bat "C:\Users\svc-dentappsnd-sftp\.ssh" "S999E-2026-mm-dd--nnn.jpg"`

- *CLI Command Notes*:
  - Use current month and day - and append a unique sequence for each test.
  - S999A - S999E are test and "lost" card folders

#### 4. Update the Main Application

- There are two new versions - Both were created by the Delphi Compiler
  - simlabxray32-2026
  - simlabxray64-2026

- Only the 32bit version has been tested. 
- The following steps are recommended:
  1) Copy the 32bit version into C:\Simlab\ folder
  2) Update the shortcut on the desktop to point to this executable
  3) OPTIONAL: Remove the simlabxray-2024.exe from this folder (delete or move)
